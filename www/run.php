...
if (isset($type)) {
	if ($type == 'searchSinger') {
		// 搜索歌手
		$ret = $api->searchSinger($_GET['singerName']);
		echo json_encode($ret, JSON_UNESCAPED_UNICODE)."\n";

	}

	if ($type == 'getMusicFavNum') {
		// 获取歌曲收藏数
		$ret = $api->getMusicFavNum($_GET['singerMID']);
		echo json_encode($ret, JSON_UNESCAPED_UNICODE)."\n";
	}

	if ($type == 'getAlbumlistBySinger') {
		// 歌手专辑列表
		$ret = $api->getAlbumlistBySinger($_GET['singerMID'], 1, 10);
		echo json_encode($ret, JSON_UNESCAPED_UNICODE)."\n";
	}

	if($type =='getSingerMusic'){
		//获取歌手歌曲列表
		$ret = $api->getSonglistBySinger($_GET['singerMID'], 1, 200);
		echo json_encode($ret, JSON_UNESCAPED_UNICODE)."\n";
	}
}
